...
CMD="bin/hdfs --daemon start datanode && ~/server/dikeHDFS"

# Make sure volume1 exists
mkdir -p ${ROOT_DIR}/opt/volume$1
...
if [ "$#" -ge 2 ] && [ $2 = "-d" ]; then
  ...
  echo "bin/hdfs --daemon start datanode"
...
docker run --rm=true $DOCKER_INTERACTIVE_RUN \
  ...
  -v "${ROOT_DIR}/opt/volume$1:/opt/volume" \
  -w "${DOCKER_HOME_DIR}/server/hadoop/hadoop" \
  -v "${ROOT_DIR}/build/.m2:${DOCKER_HOME_DIR}/.m2" \
  -v "${ROOT_DIR}/build/.gnupg:${DOCKER_HOME_DIR}/.gnupg" \
  -u "${USER_ID}" \
  --network dike-net \
  --name datanode$1 --hostname datanode$1\
